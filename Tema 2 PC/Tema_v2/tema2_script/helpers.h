...
int add_list(int value1, char *value2, Node *head) {
	Node new_node = malloc(sizeof(struct List));
	new_node->next = NULL;
	new_node->value1 = value1;
	strcpy(new_node->value2, value2);
	...
	if (*head == NULL) {
		*head = new_node;
		...
		it = *head;
		if (strcmp((*head)->value2, value2) == 0)
			return 0;
		while (it->next != NULL) {
			it = it->next;
			if (strcmp(it->value2, value2) == 0) {
				return 0;
			}
		}
		it->next = new_node;
...
int find_id_socket_table(char* id, socket_table table) {
	for (int i = 0; i < SIZE; i++) {
		...
		while (entry != NULL) {
			// printf("%d aici ar trebui sa dea\n", i);
			// printf("%s %s\n", id, entry->id);
			if (strcmp(id, entry->id) == 0)
				return 1;
			// printf("la strcmp-ul ala dadea\n");
			entry = entry->next;
			// printf("aici nu mai da\n");
		}
	}
	return 0;
}
...
// TODO - completeaza aici functii pentru adaugare elemente
